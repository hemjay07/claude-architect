...
flowchart TD
    ...
    style Crit fill:#1a56db,color:#fff,stroke:#1a56db
    style RAG fill:#0f6e56,color:#fff,stroke:#0f6e56
    style DB fill:#0f6e56,color:#fff,stroke:#0f6e56
...
[RAG] Index ready.
Connected. Tool registry: db__list_tables, db__query_db, ..., rag__search, rag__get_sources
...
src/
  ...
  mcp_client/         # Multi-server client — namespaced tool registry (db::, fs::, rag::)
...
evals/
  ...
  judge.ts            # LLM-as-a-Judge — structured rubric, deterministic safety overrides
...
literature/
  glp1/               # 7 trial summaries
  mechanisms/         # 3 mechanism files
  safety/             # 3 safety files
COST_ARCHITECTURE.md  # Cost analysis — real usage data, model selection, scaling projections
.claude/
...
Real usage from the Anthropic dashboard: **$15.31** total across ~35 queries. **$0.44 per query** average using Sonnet, zero caching.

Key findings from [COST_ARCHITECTURE.md](./COST_ARCHITECTURE.md):
- Output tokens are 14% of volume but 43% of cost — output verbosity is the highest-leverage optimisation
- BP1 caching on system prompt + tools would save 78% on those tokens
- Circuit breakers are cost containment — each additional Critic iteration roughly doubles input cost
- Post-RAG: most queries resolve in 1 iteration instead of 1.8 average
